...
/** <b>Polinomio de Grado 2 de Simpson</b>
	...
	Es más preciso que Trapecio porque usa un polinomio de 3 pares [x,f(x)]
...
public class SimpsonG2 {
	...
	public double calcular() {
		...
		System.out.println("I2(f) en ["+x[0]+","+x[2]+"] = (h/3) * ( f(x0) + 4.f(x1) + f(x2) )");
		System.out.println("I2(f) en ["+x[0]+","+x[2]+"] = ("+h+"/3) * ( "+f[0]+" + 4."+f[1]+" + "+f[2]+" )");
		double resultado = (h/3) * ( f[0] + 4 * f[1] + f[2] );
		System.out.println("I2(f) en ["+x[0]+","+x[2]+"] = " + resultado);
		return resultado;
	}
